...
export class SnowflakeID {
   ...
   * @param epoch to be used for deconstructing.
   */
  public constructor(
    snowflake: string = SnowflakeID.generate(),
    epoch: number = SnowflakeID.EPOCH,
  ) {
    ...
    this.timestamp = Number(this.snowflake >> SnowflakeID.TIME_SHIFT) + epoch;
   ...
   * ```
   *
   * @param epoch to be used for generating.
   * @external https://en.wikipedia.org/wiki/SnowflakeID_ID
   * @returns snowflake
   */
  public static generate(epoch: number = SnowflakeID.EPOCH): string {
    const time = Date.now();
    ...
    // construct snowflake
    let snowflake = BigInt(time - epoch) << SnowflakeID.TIME_SHIFT;
